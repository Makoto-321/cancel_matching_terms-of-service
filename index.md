...
この利用規約（以下，「本規約」といいます。）は，カケハビ（以下，「当社」といいます。）がこのウェブサイト上で提供する飲食店キャンセルマッチングサービス（以下，「本サービス」といいます。）の利用条件を定めるものです。登録ユーザーの皆さま（以下，「ユーザー」といいます。）には，本規約に従って，本サービスをご利用いただきます。<br>
...
- 本サービスはAI（Anthropic社のAPI）を使用してネクストアクションを生成します。AIが生成する内容は参考情報であり, その正確性・適切性を保証するものではありません。AIの生成内容に基づくユーザーの行動によって生じた損害について, 当社は一切の責任を負いません。<br>
